...
<body>
<form id="fenye" name="fenye" action="<%=request.getContextPath()%>/background/testData/find.html" method="post">
                    ...
                    <td>
	                    <div class="box_bottom pb5 pt5 pr10" style="border-top:0px solid #dadada;">
			              <div class="search_bar_btn" style="text-align:right;">
	                         ...
	                         <sec:authorize ifAnyGranted="ROLE_test_data_add">
	                            <input type="button" name="button" class="btn btn82 btn_add" value="新增" onclick="GOTO('<%=request.getContextPath()%>/background/testData/addUI.html')">
	                         </sec:authorize>
	                         <sec:authorize ifAnyGranted="ROLE_test_data_delete">
            ...
            <div class="box span10 oh">
              <table class="list_table" width="100%" cellspacing="1" onmouseover="changeto()"  onmouseout="changeback()">
			          ...
			          <c:forEach var="key" items="${pageView.data}" >
						...
						<td align="center" ><a href="<%=request.getContextPath()%>/background/testData/findById.html?objId=${key.id}&type=0">${key.id}</a></td>
						<td align="center" >${key.name}</td>
						<td align="center" >${key.mobile}</td>
						<td align="center" >${key.amount}</td>
						<td align="center" >${key.city}</td>
						<td align="center" >${fn:substring(key.createTime,0,19)}</td>
						<td align="center" >
						
						<sec:authorize ifAnyGranted="ROLE_test_data_info">
							<img src="<%=request.getContextPath()%>/images/admin.gif" width="10" height="10" />
							<a href="<%=request.getContextPath()%>/background/testData/findById.html?objId=${key.id}&type=0">
							详细信息</a>
						</sec:authorize>
						<sec:authorize ifAnyGranted="ROLE_test_data_edit">
							<img src="<%=request.getContextPath()%>/images/edt.gif" width="12" height="12" />
							<a href="<%=request.getContextPath()%>/background/testData/findById.html?objId=${key.id}&type=1">编辑</a>
						</sec:authorize>
						<sec:authorize ifAnyGranted="ROLE_test_data_delete">
							<img src="<%=request.getContextPath()%>/images/del.gif" width="16" height="16" />
							<a href="javascript:void(0);" onclick="deleteId('<%=request.getContextPath()%>/background/testData/deleteById.html','${key.id}')">删除</a>
						</sec:authorize>
